...
extern uint32_t RGB888;////将原始数据转化为RGB888格式
extern uint16_t RGB565;////将原始数据转化为RGB565格式
extern uint8_t R_Dat,G_Dat,B_Dat;//R,G,B


uint8_t TCS34725_Init(void);
RGB TCS34725_Get_RGBData(void);
void integrationTime(float ms); // 2.4 - 614.4 ms
uint16_t TCS34725_GetRGB565(RGB rgb);//将原始RGB值转换为RGB565格式
uint32_t TCS34725_GetRGB888(RGB rgb);//将原始RGB值转换为RGB888格式

void OLED_printf( uint8_t *P_data,uint8_t Location,uint16_t temp_data,uint8_t Length);//距离数据填到Dis_Buf中
